package com.zzmx.example.dataimport.util;

/**
 * ${DESCRIPTION}
 *
 * @CreateUser Kevin.Zhou
 * @CreateTime 2018/1/26 20:13
 * @ModifyUser Kevin.Zhou
 * @ModifiedTime 2018/1/26 20:13
 * @Version 1.0
 */


import com.zzmx.example.dataimport.util.handler.CellReadHandler;
import com.zzmx.example.dataimport.util.handler.DefaultPropertyWriteWriteHandler;
import com.zzmx.example.dataimport.util.handler.PropertyWriteHandler;
...
public class ExcelUtils {
    ...
    public static <T> void readExcelWithTitle(InputStream inputStream, String sheetName, Class<T> cls, List<String> columns, Collection<T> dataList, CellReadHandler<T> readHandler) throws RuntimeException {
        ...
        try {
            ...
            Sheet sheet = workbook.getSheet(sheetName);
             ...
             readSheetWithTitle(sheet, cls, columns,dataList, readHandler);
        } catch (Exception e) {
    ...
    public static <T> List<T> readExcelWithTitle(InputStream inputStream, int sheetIndex, Class<T> cls, List<String> columns, Collection<T> dataList) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, IOException {
        return readExcelWithTitle(inputStream, sheetIndex, cls, columns, dataList);
    }

    public static <T> void readExcelWithTitle(InputStream inputStream, int sheetIndex, Class<T> cls, List<String> columns, Collection<T> dataList, CellReadHandler<T> readHandler) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, IOException {
        ...
        if (dataList == null) {
            throw new RuntimeException("The parameter[dataList] must not null ");
        }
        Workbook workbook = null;
        try {
            workbook = WorkbookFactory.create(inputStream);
    ...
    private static <T> void readSheetWithTitle(Sheet sheet, Class<T> cls, List<String> columns, Collection<T> dataList, CellReadHandler<T> readHandler) throws IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, IOException {
        ...
        for (int i = firstRowNum + 1; i <= lastRowNum; i++) {
            ...
            for (int j = 0; j < columns.size(); j++) {
                ...
                if (readHandler != null && readHandler.support(fieldName)) {
                    readHandler.readValue(cell, bean, fieldName);
                    readHandler.onReaded(cell,bean,fieldName);
                }
            }
            dataList.add(bean);
        }
            ...
            for (int i = 0; i < properties.length; i++) {
                ...
                try {
                    ...
                    if (writeHandler != null && writeHandler.support(fieldName)) {
                        writeHandler.writeValue(cell, t, fieldName);
                    } else {
                        defaultPropertyWriteHandler.writeValue(cell, t, fieldName);
                    }
